...
后果：TOTP 与密码属于 User（全局一套）；因此 **Tenant 侧不得重置全局 TOTP**，丢失恢复能力由 PlatformOperator 处理。无 Membership 的 User 仍保留，仅能持 UserContext 处理安全设置与新邀请。再次加入 Tenant 一律新建 Membership，避免两段任职与旧角色混淆。
